...
Our main motivation for selecting the dataset **Amazon consumer Behaviour Dataset** that we came across in Kaggle was to unveil some customer insights which can be used for enhancing the customer experiance or the improving the business implementation after analyzing this dataset.
...
Once the data is clean, we will employ Statistical Analysis tools such as Logistic Regression or Classification to assess the likelihood of purchasing product categories based on demographic variables (age and gender)

For visualisation, we will be using heatmaps and bar charts to illustrate the relationship between the target variables.
